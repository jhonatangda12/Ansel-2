...
function imprimirFormulario() {
  ...
  const nome = document.getElementById("nome").value || "N/A"; // Valor padrão caso não preenchido
  const taxa1 = parseFloat(document.getElementById("taxa1").value) || 0; // Garantir valor numérico
  ...
  conteudoImpressao += `
      <p style="font-size: 20px; text-align: center;"><strong>Taxa: ${taxa1.toLocaleString('pt-BR')}</strong></p>
...
function calcularResultados() {
  ...
  const pagar = totalComTaxa * 0.107;

  // Exibir os resultados nos elementos HTML
  document.getElementById("resultadoTaxas").textContent = taxa1.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,});
  document.getElementById("resultadoSoma").textContent = totalComTaxa.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  document.getElementById("valorAPagar").textContent = pagar.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
}
